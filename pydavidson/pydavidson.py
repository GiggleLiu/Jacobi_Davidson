...
class _JDLinear(lin.LinearOperator):
    ...
    def __init__(self, lprojector, spmat, rprojector=None):
        ...
        self.spmat = spmat
        spshape = spmat.shape if not isinstance(
            spmat, tuple) else spmat[0].shape
        super(_JDLinear, self).__init__(shape=spshape,
                                        dtype=spmat[0].dtype if isinstance(spmat, tuple) else spmat.dtype)
...
def _jdeq_solve(A, Q, MQ, iKMQ, r, M, invK, F, shift, tol, method='lgmres', precon=False, maxiter=20):
    ...
    if not precon:
        ...
        precon = _JDLinear((iKMQ.dot(invF), MQH),
                           invK if invK is not None else sps.identity(N))
    ...
    if method == 'cg' or method == 'bicg' or method == 'bicgstab':
        if method == 'cg':
            ...
            solver = lin.bicgstab
        z, info = solver(system_matrix, right_hand_side,
                         tol=tol, M=precon, maxiter=maxiter, x0=x0)
...
def davidson_Basic(A, v0=None, tol=1e-10, maxiter=1000):
    ...
    for i in range(maxiter):
        ...
        G = bmat([[G, V.T.conj().dot(Av)], [
                 Av.T.conj().dot(V), Av.T.conj().dot(z)]])
...
def JDh(A, v0=None, k=1, which='SL', M=None, K=None, tol=1e-10, maxiter=1000, projector=None,
    ...
    if sigma is None and v0 is None and which == 'SL':
        raise ValueError(
            'You must specify a desired energy or initial vector in \'SL\' mode!')
    N = A.shape[0]
    A = A.tocsr()
    if M is not None:
        M = M.tocsr()
    ...
    if K is not None:
        t0 = time.time()
        luobj = lin.splu(K.tocsc(), permc_spec='MMD_AT_PLUS_A', options={
                         'SymmetricMode': True, 'ILU_MILU': 'SILU', 'ILU_DropTol': 1e-4, 'ILU_FillTol': 1e-2})
        invK = lin.LinearOperator(
            (N, N), matvec=luobj.solve, matmat=luobj.solve, dtype=A.dtype)
        t1 = time.time()
        if iprint > 0:
    ...
    G = v0.T.conj().dot(Av)
    theta = G[0, 0]
    if which == 'SL' and sigma is None:
        sigma = theta  # initialize sigma by v0!
    # the eigenvalues and eigenvectors.
    conv_steps, lambs, Q, F = [0], [], zeros([N, 0]), zeros([0, 0])
    ...
    for i in range(maxiter):
        ...
        while True:
            ...
            F_ = bmat([[F, MQ.T.conj().dot(iKMu)], [
                      iKMu.T.conj().dot(MQ), Mu.T.conj().dot(iKMu)]])
        ...
        z = mgs(z, Q_, MQ=MQ_)
        # z=icgs(z,Q_,M=M)
        # orthogonal to search space V.
        z, z_norm = icgs(z, V, M=M, return_norm=True)
        z = z / z_norm
        ...
        V = concatenate([V, z], axis=1)
    warnings.warn(
        'Maximum iteraction reached, but no convergent pair(Err->%s)!' % cur_tol)
    lambs.append(theta)
